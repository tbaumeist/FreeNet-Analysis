...
while read line
do
	echo "The line blah $line"
	
done << $(mysql -u root -p$password freenet -Bse "select word, location, ABS(location - $location) as diff from RandomData order by diff LIMIT 0,10 ;")
