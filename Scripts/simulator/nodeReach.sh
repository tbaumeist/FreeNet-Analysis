...
_output2="$_userDir/analysis.dat.txt"
...
function run2 
{
		...
		echo "java -cp $_scriptDir/../../bin/freenet-route-prediction.jar  frp.gephi.rti.AttackNodeAnalysisSingle -t $3 -n $networkSize -p $degree -htl 4 -ds 0 -o graph.analysis.dat -maxhtl 4"
		...
		do
			...
			then
			  ...
			  echo "$_outputHeader $innerHeader" > "$_output2"
			  _firstRunInner=1
			fi 
			 #skip over firt line it is just the headers
			if [ $innerCount -eq 1 ]
        ...
        do
          ...
          do
            ...
            do
              ...
              run2 "$command" "$values" "$networkSize.$degree.$topType.dot"
            done
          done
        done
      done
